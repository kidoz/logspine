...
template <signed_integral_not_bool T> [[nodiscard]] inline field kv(std::string_view key, T value) {
  return field(std::string(key), static_cast<std::int64_t>(value));
}

template <unsigned_integral_not_bool T> [[nodiscard]] inline field kv(std::string_view key, T value) {
...
template <std::floating_point T> [[nodiscard]] inline field kv(std::string_view key, T value) {
